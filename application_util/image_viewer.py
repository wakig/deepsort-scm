...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def drawrect(self,x,y,w,h,thickness=1,style='dotted',label=None):
        pt1 = int(x), int(y)
        pt2 = int(x + w), int(y + h)
        pts = [pt1,(pt2[0],pt1[1]),pt2,(pt1[0],pt2[1])] 
        self.drawpoly(self.image,pts,self._color,self.thickness,style)
        if label is not None:
            text_size = cv2.getTextSize(
                label, cv2.FONT_HERSHEY_PLAIN, 1, self.thickness)
    ...
    def arrowed_line(self, start_point, end_point, color, thickness):
        cv2.arrowedLine(self.image,start_point,end_point,color,thickness)