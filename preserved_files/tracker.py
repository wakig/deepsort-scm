...
class Tracker:
    ...
    def _match(self, detections):
        ...
        def gated_metric(tracks, dets, track_indices, detection_indices):
            ...
            cost_matrix_a = self.metric.distance(features, targets)       # for the appearance features and gating
            cost_matrix_a[cost_matrix_a > self.metric.matching_threshold] = INFTY_COST
            cost_matrix_a *= (1.0-lam)
            cost_matrix_b = np.zeros((cost_matrix_a.shape[0],cost_matrix_a.shape[1]))
            cost_matrix_b = linear_assignment.gate_cost_matrix(           # for the bbox gating
                self.kf, cost_matrix_b, tracks, dets, track_indices,
                detection_indices)
            cost_matrix_b *= lam

            return cost_matrix_a + cost_matrix_b
